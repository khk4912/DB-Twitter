...
public class Follow {
    ...
    public void execute(String followID, String followerID) {
        try {
            ...
            checkPstmt.setString(1, followID); // 팔로우 하는 사람
            checkPstmt.setString(2, followerID); // 팔로우 받는 사람
            ResultSet rs = checkPstmt.executeQuery();

            if (!rs.next()) {
                String insertSql = "INSERT INTO follower VALUES (?, ?)";
                PreparedStatement insertPstmt = con.prepareStatement(insertSql);
                insertPstmt.setString(1, followID); // 팔로우 하는 사람
                insertPstmt.setString(2, followerID); // 팔로우 받는 사람
                insertPstmt.executeUpdate();

                String insertSql2 = "INSERT INTO following VALUES (?, ?)";
                PreparedStatement insertPstmt2 = con.prepareStatement(insertSql2);
                insertPstmt2.setString(1, followerID); // 팔로우 받는 사람
                insertPstmt2.setString(2, followID); // 팔로우 하는 사람
